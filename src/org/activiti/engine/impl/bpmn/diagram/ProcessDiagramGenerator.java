		...
		activityDrawInstructions.put("subProcess", new ActivityDrawInstruction() {
			...
			public void draw(ProcessDiagramCanvas processDiagramCreator, ActivityImpl activityImpl) {
				...
				if (isExpanded != null && isExpanded == false) {
					processDiagramCreator.drawCollapsedSubProcess((String) activityImpl.getProperty("name"), activityImpl.getX(), activityImpl.getY(), activityImpl.getWidth(), activityImpl.getHeight());
				} else {
					processDiagramCreator.drawExpandedSubProcess((String) activityImpl.getProperty("name"), activityImpl.getX(), activityImpl.getY(), activityImpl.getWidth(), activityImpl.getHeight());
				}
			}
		});
	...
	protected static void drawActivity(ProcessDiagramCanvas processDiagramCanvas, ActivityImpl activity, List<String> highLightedActivities) {
		...
		for (PvmTransition sequenceFlow : activity.getOutgoingTransitions()) {
			...
			for (int i = 2; i < waypoints.size(); i += 2) { // waypoints.size()
				...
				boolean drawText = sequenceFlow.getProperty("condition") != null;
				if (drawText) {
